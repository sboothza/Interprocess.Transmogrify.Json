...
namespace Interprocess.Transmogrify.Json.Tests
{
	public class ModelUpdate<T>
	{
		...
		[Interprocess.Transmogrify.Json.JsonProperty("<OldItem>k__BackingField")]
		...
		[Interprocess.Transmogrify.Json.JsonProperty("<NewItem>k__BackingField")]
	...
	public class SampleChild
	{
		...
		[Interprocess.Transmogrify.Json.JsonProperty(Ignore = true)]
	...
	public class Tests
	{
		...
		{
			var json = Interprocess.Transmogrify.Json.JsonSerializer.Serialize(_event.LocationInformation);
		...
		public void MeasureCustomSerialize()
		{
			...
			{
				var json = Interprocess.Transmogrify.Json.JsonSerializer.Serialize(_event, new JsonSerializerOptions());
				if (json.Length < 0)
		...
		public void TestError()
		{
			...
			try
			{
				var json = Interprocess.Transmogrify.Json.JsonSerializer.Serialize(update, new JsonSerializerOptions
				{
					IgnoreErrors = false
				});
				ClassicAssert.IsTrue(false, "should not pass");
			}
			catch
			{
				ClassicAssert.IsTrue(true, "should fail here");
			}
			...
			try
			{
				...
				ClassicAssert.IsTrue(true, "should pass");
				ClassicAssert.IsNotEmpty(json);
			}
			catch
			{
				ClassicAssert.IsTrue(false, "should not fail here");
			}
		}
		...
		public void TestNotPretty()
		{
			...
			var json = Interprocess.Transmogrify.Json.JsonSerializer.Serialize(item, new JsonSerializerOptions
			{
				IgnoreErrors = true
			});
			var jsonToMatch =
		...
		public void TestNulls()
		{
			...
			json = Interprocess.Transmogrify.Json.JsonSerializer.Serialize(item, new JsonSerializerOptions
			{
				IgnoreErrors = true,
				DontSerializeNulls = true
			});
		...
		public void TestRename()
		{
			...
			json = Interprocess.Transmogrify.Json.JsonSerializer.Serialize(update, new JsonSerializerOptions
			{
				IgnoreErrors = true,
		...
		public void TestDeserialize()
		{
			...
			var item = Interprocess.Transmogrify.Json.JsonSerializer.Deserialize<ModelUpdate<BasicItem>>(json, new JsonSerializerOptions
			{
				IgnoreErrors = true
			});
		...
		public void MeasureCustomDeserialize()
		{
			...
			for (var i = 0; i < LOOPS; i++)
			{
				var obj = Interprocess.Transmogrify.Json.JsonSerializer.Deserialize<ModelUpdate<BasicItem>>(json, options);
